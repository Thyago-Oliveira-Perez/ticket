...
| **Fraud provider**    | Java     | Fake fraud/identity check                                             |
...
The external providers (Payment in Go, Fraud in Java, Notification in Node.js) simulate the real world's unreliability. Each exposes configuration for injected latency, random `500`s, rate limiting, duplicate webhooks, and out-of-order webhook delivery. The .NET side is built to survive all of it — that's the whole reason they exist. They also model the *other* side of an integration: how to design an external API that behaves under stress.
...
Observability is end-to-end: OpenTelemetry tracing across the .NET, Go, Java, and Node.js services, Prometheus + Grafana metrics, and structured logs correlated by a single id, so one checkout can be followed across every hop.
...
- **External providers:** Go (Payment) · Java (Fraud) · Node.js (Notification)
...
3. External providers with failure injection (Go for Payment, Java for Fraud, Node.js for Notification).
